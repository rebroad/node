...
CONTAINER_IMAGE="dockcross/linux-x86:latest"
...
Cross-build Node.js for Debian 12 i386 (orchid target).
...
Options:
  ...
  --container-image IMAGE    Base container image (default: dockcross/linux-x86:latest)
  --container-platform P     Container platform (default: linux/amd64)
  --container-name NAME      Persistent build container name
  ...
  --no-builder-image-cache   Use base image directly (skip builder image creation)
...
ensure_binfmt_if_needed() {
  ...
  if [ "$CONTAINER_INSTALL_BINFMT" -eq 1 ] && [ "$runtime" = "docker" ]; then
    ...
    sudo docker run --privileged --rm tonistiigi/binfmt --install i386 >/dev/null
...
derive_builder_image() {
  ...
  printf 'node-orchid-builder:%s' "$image_key"
}

derive_work_container() {
  image_key="$(printf '%s' "$CONTAINER_IMAGE" | tr '/:@' '___' | tr -cd 'A-Za-z0-9._-')"
  src_key="$(basename "$NODE_DIR" | tr '/:@' '___' | tr -cd 'A-Za-z0-9._-')"
  printf 'node-orchid-work-%s-%s' "$src_key" "$image_key"
}
...
RUN set -eu; \
    ...
    apt-get install -y --no-install-recommends \
      ...
      python3 xz-utils zlib1g-dev libssl-dev libffi-dev libbz2-dev \
...
run_container_build() {
  ...
  run_image="$2"
  work_container="$3"
  ccache_dir="${CCACHE_DIR:-$HOME/.ccache-node-i386}"
  run_uid="$(id -u)"
  run_gid="$(id -g)"
  ...
  build_cmd='
      ...
      if [ -f out/deps/zlib/zlib.target.mk ]; then
        sed -i "/DEFLATE_SLIDE_HASH_SSE2/d;/INFLATE_CHUNK_SIMD_SSE2/d;/ADLER32_SIMD_SSSE3/d" out/deps/zlib/zlib.target.mk
        sed -i "/deps\\/zlib\\/cpu_features\\.o/d" out/deps/zlib/zlib.target.mk
      fi
      if ls out/deps/zlib/*.target.mk >/dev/null 2>&1; then
        sed -i "s/-m32 \\\\/-m32 -msse2 -mfpmath=sse -mmmx \\\\/g" out/deps/zlib/*.target.mk
      fi
